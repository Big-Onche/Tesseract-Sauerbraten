...
//            particle_fireball(b.ammopos, 4.8f, PART_EXPLOSION, 0, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 4.8f);
            ...
            if(m_regencapture)
            {
                ...
                rendermodel(ammoname, animFlags, ammopos, lastmillis/10.0f, 0, 0, mdlFlags);
            }
            else loopj(b.ammo)
            {
                ...
                ammopos.z += 4;
                rendermodel(entities::entmdlname(I_SHELLS+b.ammotype-1), animFlags, ammopos, 0, 0, 0, mdlFlags);
            }
